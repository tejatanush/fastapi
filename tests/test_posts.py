...
def test_unauthorized_user_create_post(client,test_user,test_posts):
    res=client.post(
        "/posts/",json={"title":"arbitary title","content":"abc"}
    )
    assert res.status_code==401
...
'''def test_unauthorized_user_update_post(authorized_client,test_user,test_posts):
    data = {
        "title": "this title should not be updated",
        "content": "this content should not be updated",
        "id":test_posts[0].id
    }
    res=authorized_client.put(f"/posts/{test_posts[0].id}",json=data)
    #updated_post=schemas.Post(**res.json())
